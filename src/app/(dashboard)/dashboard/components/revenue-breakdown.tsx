  ...
  return (
    <Card data-chart={id} className="flex flex-col cursor-pointer">
      ...
      <CardHeader className="flex flex-col space-y-2 sm:flex-row sm:items-center sm:justify-between sm:space-y-0 pb-2">
        <div>
          <CardTitle>Repartition du chiffre d'affaires</CardTitle>
          <CardDescription>Distribution du CA par canal</CardDescription>
        </div>
        <div className="flex items-center space-x-2">
          <Select value={activeCategory} onValueChange={setActiveCategory}>
            <SelectTrigger
              className="w-43.75 rounded-lg cursor-pointer"
              aria-label="Selectionner un canal"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full">
          <div className="flex justify-center">
            <ChartContainer
              id={id}
              config={chartConfig}
              className="mx-auto h-75 min-w-0 w-full max-w-75"
            >
              <PieChart>
                <ChartTooltip
                  cursor={false}
